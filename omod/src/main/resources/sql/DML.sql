...
(CASE when o.concept_id in(5497,730,654,790,856) then o.value_numeric
	when o.concept_id in(1030,1305,1325,159430,161472,1029,1031,1619,1032,162202,307,45,167718,163722,167452,167459,1643,32,1366) then o.value_coded
	END) AS test_result,
...
inner join obs o on e.encounter_id=o.encounter_id and o.voided=0 and o.concept_id in (5497,730,654,790,856,1030,1305,1325,159430,161472,1029,1031,1619,1032,162202,307,45,167718,163722,167452,167459,1643,32,1366)
...
CREATE PROCEDURE sp_populate_etl_prep_enrolment()
  BEGIN
    ...
    from encounter e
      ...
      inner join obs o on o.encounter_id = e.encounter_id and o.concept_id in (164932,160540,162724,161550,160534,160535,160555,159599,160533,1088,166866,162881,5629,160638,165038,160640,160642,160641,164930,160581) and o.voided=0
    where e.voided=0
    group by e.encounter_id;
...
select o3.uuid                                                                             as uuid,
       ...
       max(
         if(o1.concept_id = 1410 and o1.obs_group = 1184, date(o1.value_datetime),
            null))                                                                           as date_taken_booster_vaccine,
...
-- Procedure sp_populate_etl_clinical_encounter --
