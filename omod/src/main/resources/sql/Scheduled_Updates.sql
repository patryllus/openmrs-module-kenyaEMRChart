    ...
    insert into kenyaemr_etl.etl_laboratory_extract(
      ...
      select
        ...
        (case when o.concept_id in(5497,730,654,790,856) then o.value_numeric
         when o.concept_id in(1030,1305,1325,159430,161472,1029,1031,1619,1032,162202,307,45,167718,163722,167452,167459,1643,32,1366) then o.value_coded END) AS test_result,
...
select
    ...
    e.voided
      from encounter e
        ...
        5272,5632,162699,1788,159803,160632,164126,159803,167229,160540,167163,167162,160416,1396,5569,164956)
      and o.voided=0
      where e.voided=0
            and e.date_created >= last_update_time
            or e.date_changed >= last_update_time
